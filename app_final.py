...
def main():
    ...
    st.markdown("""
        ...
        <p style='text-align: center; color: #64748B; margin-bottom: 1.5rem; font-size: 0.875rem;'>
            Travel Flexibility Survey Analysis
        </p>
    ...
    with col2:
        ...
        if selected_segments:
            ...
            # Allow response selection
            with st.expander("📊 Filter Responses (Interactive Table)"):
                st.markdown("**Select responses to display in chart:**")

                # Use multiselect for response filtering
                all_responses = df["Response"].tolist()
                selected_responses = st.multiselect(
                    "Responses to show",
                    options=all_responses,
                ...
                if selected_responses:
                    ...
                    for col in filtered_df.columns:
                        if " %" in col:
                            filtered_df[col] = filtered_df[col].apply(lambda x: f"{x:.1f}%")
                    ...
                    st.warning("Please select at least one response to display")

            # Chart (updated based on selected responses)
            if selected_responses:
                fig = create_chart(
                    current_question,
                    selected_segments,
